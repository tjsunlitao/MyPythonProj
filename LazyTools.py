...
2. 大兵插件
3. 常用封装
Created on 2017年9月6日10:49:00
@author: SLT
Email: octalempyrean@gmail.com
'''
# }}}
...
# }}}
    ...
    def FindPic(self, x1, y1, x2, y2, pic_fullname_list):
        ...
        '''
        assert(isinstance(pic_fullname_list, list)
               or isinstance(pic_fullname_list, tuple))
        ss = self._dm.FindPicE(self._rx + x1, self._ry + y1, self._rx + x2,
                               self._ry + y2, '|'.join(pic_fullname_list),
                               self._delta_color, self._sim, self._direction)
        s = ss.split('|')
        return (s[0] != '-1', int(s[0]), int(s[1]) - self._rx,
                int(s[2]) - self._ry)
    ...
    def FindPicEx(self, x1, y1, x2, y2, pic_fullname_list):
        ...
        比如"0,100,20|2,30,40" 表示找到了两个,第一个,对应的图片是图像序号为0的图片,
            坐标是(100,20),第二个是序号为2的图片,坐标(30,40)
        '''
        assert(isinstance(pic_fullname_list, list)
               or isinstance(pic_fullname_list, tuple))
        sss = self._dm.FindPicEx(self._rx + x1, self._ry + y1, self._rx + x2,
                                 self._ry + y2, '|'.join(pic_fullname_list),
                                 self._delta_color, self._sim, self._direction)
    ...
    def Capture(self, x1, y1, x2, y2, fileName):
        ...
        '''
        if self._dm.Capture(x1 + self._rx, y1 + self._ry, x2 + self._rx,
                            y2 + self._ry, fileName) == 0:
    ...
    def SetDict(self, i, dic_file, color_format, sim):
        ...
        s = dm.Ocr(0,0,2000,2000,
                   "9f2e3f-030303|2d3f2f-000000|3f9e4d-100000",1.0)
        MessageBox s
    ...
    def FindStr(self, x1, y1, x2, y2, strs):
        ...
        # print(self._rx + x1, self._ry + y1, self._rx + x2, self._ry + y2,
        #       '|'.join(strs), self._color_formats[self._dicIndex],
        #       self._sims[self._dicIndex])
        ss = self._dm.FindStrE(self._rx + x1, self._ry + y1, self._rx + x2,
                               self._ry + y2, '|'.join(strs),
                               self._color_formats[self._dicIndex],
                               self._sims[self._dicIndex])
        s = ss.split('|')
        return (s[0] != '-1', strs[int(s[0])], int(s[1]) - self._rx,
                int(s[2]) - self._ry)
    ...
    def FindStrEx(self, x1, y1, x2, y2, strs):
        ...
        '''
        assert(isinstance(strs, list) or isinstance(strs, tuple))
        sss = self._dm.FindStrFastE(self._rx + x1, self._ry + y1,
                                    self._rx + x2, self._ry + y2,
                                    '|'.join(strs),
                                    self._color_formats[self._dicIndex],
                                    self._sims[self._dicIndex])
        rr = []
    ...
    def Ocr(self, x1, y1, x2, y2):
        ...
        #       self._color_formats[self._dicIndex],
        #       self._sims[self._dicIndex])
        return self._dm.Ocr(self._rx + x1, self._ry + y1, self._rx + x2,
                            self._ry + y2, self._color_formats[self._dicIndex],
                            self._sims[self._dicIndex])
    ...
    def OcrEx(self, x1, y1, x2, y2):
        ...
        '''
        sss = self._dm.OcrEx(self._rx + x1, self._ry + y1, self._rx + x2,
                             self._ry + y2,
                             self._color_formats[self._dicIndex],
                             self._sims[self._dicIndex])
        rr = []
    ...
    def BindWindow(self, hwnd, display, mouse, keypad, mode):
        ...
        '''
        self.UnBindWindow()
        # if self._dm.BindWindow(self.Hwindow(hwnd), display,
        if self._dm.BindWindow(hwnd, display,
                               mouse, keypad, mode) == 0:
    ...
    def BindWindowEx(self, display, mouse, keypad, public, mode, hwnd=-1):
        ...
        "dx2" : dx2模式,用于窗口采用dx模式刷新,如果dx方式会出现窗口进程崩溃的状况,
            可以考虑采用这种.采用这种方式要保证窗口有一部分在屏幕外.win7或者vista不需要移动也可后台. 此模式占用CPU较大.
        "dx3" : dx3模式,同dx2模式,但是如果发现有些窗口后台不刷新时,
            可以考虑用dx3模式,此模式比dx2模式慢许多. 此模式占用CPU较大.
        dx模式,用于窗口采用dx模式刷新,取值可以是以下任意组合，组合采用"|"符号进行连接
            注意此模式需要管理员权限. 支持BindWindow中的缩写模式.
        1. "dx.graphic.2d"  2d窗口的dx图色模式
        2. "dx.graphic.2d.2"  2d窗口的dx图色模式是dx.graphic.2d的增强模式.兼容性更好.
            <收费功能，具体详情点击查看>
        3. "dx.graphic.3d"  3d窗口的dx图色模式,注意采用这个模式，必须关闭窗口3D视频设置的全屏抗锯齿选项.
        4. "dx.graphic.3d.8"  3d窗口的dx8图色模式,注意采用这个模式，必须关闭窗口3D视频设置的全屏抗锯齿选项.
            这个模式支持某些老的引擎. <收费功能，具体详情点击查看>

        mouse 字符串: 鼠标仿真模式 取值有以下几种
        "normal" : 正常模式,平常我们用的前台鼠标模式
        "windows" : Windows模式,采取模拟windows消息方式 同按键的后台插件.
        "windows2" : 代表"dx.mouse.position.lock.api|
            dx.mouse.position.lock.message|dx.mouse.state.message"
        ...
        5. "dx.mouse.clip.lock.api" 此模式表示通过封锁系统API来锁定刷新区域。
        ...
        "windows": Windows模式,采取模拟windows消息方式 同按键的后台插件.
        "dx" : 代表" dx.public.active.api|dx.public.active.message|
        ...
        2.  "dx.public.active.message" 此模式表示通过封锁系统消息来锁定窗口激活状态.
            注意，部分窗口在此模式下会耗费大量资源 慎用. 另外如果要让此模式生效，
            必须在绑定前，让绑定窗口处于激活状态,否则此模式将失效. 比如dm.SetWindowState hwnd,1 然后再绑定.
        3.  "dx.public.disable.window.position" 此模式将锁定绑定窗口位置.
            不可与"dx.public.fake.window.min"共用.
        4.  "dx.public.disable.window.size" 此模式将锁定绑定窗口,禁止改变大小.
            不可与"dx.public.fake.window.min"共用.
        5.  "dx.public.disable.window.minmax" 此模式将禁止窗口最大化和最小化,
            但是付出的代价是窗口同时也会被置顶. 不可与"dx.public.fake.window.min"共用.
        6.  "dx.public.fake.window.min" 此模式将允许目标窗口在最小化状态时，仍然能够像非最小化一样操作..
            另注意，此模式会导致任务栏顺序重排，所以如果是多开模式下，会看起来比较混乱，建议单开使用，多开不建议使用.
            <收费功能，具体详情点击查看>
        7.  "dx.public.hide.dll" 此模式将会隐藏目标进程的大漠插件，避免被检测..
            另外使用此模式前，请仔细做过测试，此模式可能会造成目标进程不稳定，出现崩溃。<收费功能，具体详情点击查看>
        ...
        9.  "dx.public.input.ime" 此模式是配合SendStringIme使用. 具体可以查看SendStringIme接口.
            <收费功能，具体详情点击查看>
        10. "dx.public.graphic.protect" 此模式可以保护dx图色不被恶意检测.
            同时对dx.keypad.api和dx.mouse.api也有保护效果.
            <收费功能，具体详情点击查看>
        11. "dx.public.disable.window.show" 禁止目标窗口显示,
            这个一般用来配合dx.public.fake.window.min来使用.
            <收费功能，具体详情点击查看>
        12. "dx.public.anti.api" 此模式可以突破部分窗口对后台的保护. <收费功能，具体详情点击查看>
        13. "dx.public.memory" 此模式可以让内存读写函数突破保护.只要绑定成功即可操作内存函数.
            <收费功能，具体详情点击查看>
        14. "dx.public.km.protect" 此模式可以保护dx键鼠不被恶意检测.
            最好配合dx.public.anti.api一起使用.
            此属性可能会导致部分后台功能失效. <收费功能，具体详情点击查看>
        15. "dx.public.prevent.block"  绑定模式1 3 5 7 101 103下，可能会导致部分窗口卡死.
            这个属性可以避免卡死.<收费功能，具体详情点击查看>
        16. "dx.public.ori.proc"  此属性只能用在模式0 1 2 3和101下.
            有些窗口在不同的界面下(比如登录界面和登录进以后的界面)，键鼠的控制效果不相同.
            那可以用这个属性来尝试让保持一致. 注意的是，这个属性不可以滥用，确保测试无问题才可以使用. 否则可能会导致后台失效.
            <收费功能，具体详情点击查看>
        ...
        101 : 超级绑定模式. 可隐藏目标进程中的dm.dll.避免被恶意检测.效果要比dx.public.hide.dll好.
            推荐使用. <收费功能，具体详情点击查看>
        103 : 同模式101，如果模式101有崩溃问题，可以尝试此模式. <收费功能，具体详情点击查看>

        需要注意的是: 模式1 3 5 7 101 103在大部分窗口下绑定都没问题。
        但也有少数特殊的窗口，比如有很多子窗口的窗口，对于这种窗口，在绑定时，一定要把鼠标指向一个可以输入文字的窗口，
        比如一个文本框，最好能激活这个文本框，这样可以保证绑定的成功.
        '''
        self.UnBindWindow()
        if self._dm.BindWindowEx(self.Hwindow(hwnd), display, mouse,
                                 keypad, public, mode) == 0:
            raise DMerror(self, '绑定窗口错误！请检查')
        else:
            self._bBindWnd = True
    ...
    def FindWindowEx(self, className, title, hwnd=-1):
        ...
        '''
        self._hwnd = self._dm.FindWindowEx(self.Hwindow(hwnd),
                                           className, title)
        return self._hwnd

    def EnumWindow(self, parent, title, className="", filter=1+4+8+16):
        '''
        ...
        '''
        self._hwnd = self._dm.EnumWindow(parent, title, className, filter)
        try:
            if self._hwnd == "":
                raise DMerror(self._dm,
                              u"枚举窗口失败,请检查hwnd是否存在!")
            else:
                self._hwnd = str(self._hwnd).split(",")
        finally:
            return self._hwnd
    ...
    def Hwindow(self, hwnd=-1):
        ...
        if hwnd > 0:
            self._hwnd = hwnd
        elif self._hwnd is None or self._hwnd <= 0:
            self._hwnd = self._dm.GetForegroundWindow()
            if self._hwnd <= 0:
                raise DMerror(self, "找不到窗口，请检查hwnd是否存在！")
        return self._hwnd
    ...
    def SetWindowState(self, flag, hwnd=-1):
        '''
    ...
    def ActiveWindow(self, hwnd=-1):
        '''
        激活窗口
        '''
        self.SetWindowState(1, hwnd)
...
class DMerror(Exception):
    def __init__(self, dm, msg):
        self._dm = dm
        print (u'大漠错误（' + self.GetLastError() + u'） \n---- ' + msg)
    ...
    def GetLastError(self):
        ...
        s = u'无错误'
        if e == -1:
            s = u'表示你使用了绑定里的收费功能，但是没注册，无法使用.'
        elif e == -2:
            s = u'使用模式0 2 4 6时出现，因为目标窗口有保护，或者目标窗口没有以管理员权限打开\
                    常见于win7以上系统.或者有安全软件拦截插件.解决办法: 关闭所有安全软件,\
                    并且关闭系统UAC,然后再重新尝试. 如果还不行就可以肯定是目标窗口有特殊保护.'
        elif e == -3:
            s = u'使用模式0 2 4 6时出现，可能目标窗口有保护，也可能是异常错误.'
        elif e == -4:
            s = u'使用模式1 3 5 7 101 103时出现，这是异常错误.'
        elif e == -5:
            s = u'使用模式1 3 5 7 101 103时出现, 这个错误的解决办法就是关闭目标窗口,\
                    重新打开再绑定即可. 也可能是运行脚本的进程没有管理员权限.'
        elif e in (-6, -7, -9):
            s = u'使用模式1 3 5 7 101 103时出现,异常错误. 还有可能是安全软件的问题，比如360等。尝试卸载360.'
        elif e in (-8, -10):
            s = u'使用模式1 3 5 7 101 103时出现, 目标进程可能有保护,也可能是插件版本过老，试试新的或许可以解决.'
        elif e == -11:
            s = u'使用模式1 3 5 7 101 103时出现, 目标进程有保护. 告诉我解决。'
        elif e == -12:
            s = u'使用模式1 3 5 7 101 103时出现, 目标进程有保护. 告诉我解决。'
        elif e == -13:
            s = u'使用模式1 3 5 7 101 103时出现, 目标进程有保护. 或者是因为上次的绑定没有解绑导致。\
                     尝试在绑定前调用ForceUnBindWindow.'
        elif e == -14:
            s = u'使用模式0 1 4 5时出现, 有可能目标机器兼容性不太好. 可以尝试其他模式. 比如2 3 6 7'
        elif e == -16:
            s = u'可能使用了绑定模式 0 1 2 3 和 101，然后可能指定了一个子窗口.导致不支持.\
                    可以换模式4 5 6 7或者103来尝试. 另外也可以考虑使用父窗口或者顶级窗口.\
                    来避免这个错误。还有可能是目标窗口没有正常解绑 然后再次绑定的时候.'
        elif e == -17:
            s = u'模式1 3 5 7 101 103时出现. 这个是异常错误. 告诉我解决.'
        elif e == -18:
            s = u'句柄无效.'
        elif e == -19:
            s = u'使用模式0 1 2 3 101时出现,说明你的系统不支持这几个模式. 可以尝试其他模式.'
        return s
# }}}
...
class DB(object):
    ...
    # 初始化类# {{{
    def __init__(self,):
        self._dbPlugin = win32com.client.Dispatch('DBSoft.DBSoft_plugins')
        self._dbWindow = win32com.client.Dispatch('DBSoft.DBSoft_Window')
        self._dbMsgbox = win32com.client.Dispatch('DBSoft.DBSoft_Msgbox')
# }}}

    # 类方法# {{{

    # 软件服务(DBSoft.DBSoft_plugins)# {{{
    @property
    def CheckDBSoft(self):
        '''
        检查大兵插件的完整性
        函数返回:不完整返回0,完整返回1，检查失败返回2
        '''
        return self._dbPlugin.CheckDBSoft()

    def RegDBSoft(self):
        '''
        注册大兵插件
        函数返回:注册大兵插件，返回1表示注册成功，返回2表示注册码正确，进入试用时间1-2小时，失败返回0
        '''
        return self._dbPlugin.RegDBSoft("934AA748840647F9359BB04EF5EBB171")

    def GetDBSoftVersion(self):
        '''
        获得到当前插件版本号
        函数返回:返回字符串
        '''
        return self._dbPlugin.GetDBSoftVersion()
# }}}

    # 窗口相关(DBSoft.DBSoft_Window)

    # 消息相关(DBSoft.DBSoft_Msgbox)# {{{

    def MessageBox(self, str1, str2, buttonOption=4):
        return self._dbMsgbox.MessageBox(str1, str2, buttonOption)
# }}}
