...
域的首行左侧不能有空白字符，比如空格或者制表符，占用多行的域其后续行则必须以空白字符开头。域的信息内容中还可以包含属性，属性之间以“;”分隔，属性的格式如下：{属性名称}=”{属性值}”。    
    ...
    try:
        ...
        mailServer = smtplib.SMTP('smtp.yandex.com', 587)  # 连接腾讯邮件的服务器；SMTP（Simple Mail Transfer Protocol）即简单邮件传输协议，用于由源地址到目的地址传送邮件的规则，由它来控制信件的中转方式
...
# dumpPickle('''octalempyrean@qq.com?qmxpkqndzdwkbccj?[octalempyrean@qq.com]''', os.path.basename('.'), "senderInfo.txt") 
dumpPickle('''ceshikankan@yandex.com?iqpluatlhrwjtrpv?[octalempyrean@qq.com]''', os.path.basename('.'), "senderInfo.txt") 
if __name__ == "__main__":
    ...
    for subRecipient in recipient:
        ...
        r'''Уважаемый господин / госпожа:

        Наша компания продает все типоразмеры подшипников.
        	В мире: SKF, FAG, INA, KOYO, NSK, TIMKEN, NSK, IKO т.д.
        	В Китае: HRB, ZWZ, LYC, MKR и т.д.
        	У нас свой завод, тоже можем оказать услуги OEM.
        Цена низкая, качество высокое!!!
        И у нас богатый опыт по экспорту подшипника в России, и у нас тоже опытные перевозчики, которые могут выполнить полный процесс растаможи и доставку.
        с Китая до любого города России.
        Мы оказали гарантийное обслуживание, бесплатно заменить или вернуть бракованные подшипники.

        Надеюсь, что у нас будет возможность сотрудничества.
        Жду вашего запроса.

        Лариса
        менеджер по продажам на русском языке
        Организация: ООО Майкл.
        Сайт: www.mkrbearing.com
        факс: +86-22-58118786
        Телефон: +86 15010283459
        SKYPE: zhaodan371147793
        Почта: lisa@mkrbearing.com

        ''', 
        r'Продажа подшипников.pdf')